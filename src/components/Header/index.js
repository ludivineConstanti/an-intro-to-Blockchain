...
const Header = ({
  ...
  const handleClickMenu = () => {
    if (!isPlaying) onClickButtonMenu();
    else setShowPopUpLeave(true);
  };
  ...
  return (
    <>
      ...
      <span className="header">
        ...
        <button type="button" className="header__buttonTL" onClick={handleClickMenu}>
          { (menuOpen === true || isPlaying) ? (<>{ closeIcon }</>) : (<>{ menuIcon }</>)}
        </button>

        <div className="header__buttonTR">
          {
        !menuOpen && isLoggedIn && !isPlaying && <>{ profileIcon }</>
        }
        </div>
