export default (questionNumber, questionData, userAnswer, language) => {
  ...
  if (userTotalBadAnswers === 0) {
    if (totalGoodAnswers === userTotalGoodAnswers) {
      ...
      if (totalGoodAnswers === 1) {
        subtitle = switchL ? 'Cette réponse est correcte' : 'Thats the correct answer';
      }
    }
    else if (userTotalGoodAnswers === 1 && userTotalGoodAnswers < totalGoodAnswers) {
    ...
    else if (userTotalGoodAnswers === totalGoodAnswers - 1) {
      ...
      else if (totalGoodAnswers === 4) {
        label = switchL ? 'Presque!' : 'Almost!';
      }
    }
    else if (userTotalGoodAnswers === 0) {
  ...
  else if (userTotalBadAnswers === totalBadAnswers) {
    if (userTotalGoodAnswers === 0) {
      label = switchL ? 'Félicitations !' : 'Congratulation!';
  ...
  else if (userTotalBadAnswers === 3) {
    label = switchL ? 'Impressionnant !' : 'Amazing!';
